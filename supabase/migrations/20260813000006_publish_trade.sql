-- ONE → FIVE — PROMPT 11 / build spec §6.5, §38
-- Migration 6: private completion notes + transactional trade publishing.
--
-- Publishing a completed trade touches five tables (trades, trade_media,
-- challenge_settings, offers, email_broadcasts). It must be all-or-nothing
-- and serialized, so it lives in a single Postgres function (spec §25:
-- sensitive mutations through narrowly scoped RPCs, not browser-side write
-- sequences).
--
-- SECURITY INVOKER: the body runs with the caller's privileges, so every
-- statement still passes RLS (admin-only policies). The explicit is_admin()
-- gate is belt-and-braces; EXECUTE is also restricted to authenticated.

alter table public.trades
  add column if not exists private_completion_notes text;
